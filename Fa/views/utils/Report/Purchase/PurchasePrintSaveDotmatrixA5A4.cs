...
namespace Fa.views.utils.Report.Purchase
{
        ...
        public class PurchasePrintSaveDotmatrixA5A4
        {
            ...
            public PurchasePrintSaveDotmatrixA5A4(object sender)
            {
                fileName = (sender as PurchasePrintSaveA5A4).fileName;
            ...
            public void GenerateDotmarixTxt(IList<TaxTable> lTaxTable, DataTable dataTable, DataTable totalTable, PurchaseEntry PurchaseEntry, string PrintPaper, bool isPrint)
            {
                if (PrintPaper == "A4 LANDSCAPE")
                {
                    Line_Char = 145;
                }
                TotalLines = DotmatrixDataAlignment.PurchaseDotmatrixHeaderTable(rdr, Line_Char, PurchaseEntry);
                TotalLines += DotmatrixDataAlignment.PurchaseDotmatrixMainTable(rdr, Line_Char, PurchaseEntry);
                PrintDetails(lTaxTable, dataTable, PurchaseEntry, totalTable, PrintPaper);
                DotmatrixDataAlignment.SkipLine(rdr, 3);
                Close();
                DotmatrixPrint.DoPrint(fileName, isPrint);
            }
            public void PrintDetails(IList<TaxTable> lTaxTable, DataTable dataTable, PurchaseEntry PurchaseEntry, DataTable Total, string PrintPaper)
            {
                ...
                for (int i = 0; i < rows; i++)
                {
                    if (TotalLines > 31)
                    {
                        RunningContinueTotals(dataTable, PurchaseEntry, PrintPaper);
                    }
                    var productDetails = "";
                    int Index = 0;
                ...
                {
                    TaxDetailTable(lTaxTable, dataTable, PrintPaper, PurchaseEntry.PurchaseTaxType);
                }
            }
            ...
            private void RunningContinueTotals(DataTable dataTable, PurchaseEntry PurchaseEntry, string PrintPaper)
            {
                ...
                //next paper
                //mini header
                rdr.WriteLine(DotmatrixPrint.Bold_On + DotmatrixDataAlignment.GetFormatedText(Global.Company.Name, (Line_Char / 2), AlignmentTypes.Suffix) + DotmatrixDataAlignment.GetFormatedText((PurchaseEntry.AccountId != null ? "Customer Name:" + PurchaseEntry.SupplierName : ""), (Line_Char / 2), AlignmentTypes.Prefix) + DotmatrixPrint.Bold_Off);
                rdr.WriteLine(DotmatrixPrint.Bold_On + DotmatrixDataAlignment.GetCenterText((PurchaseEntry.PurchaseEntrytype == PurchaseEntrytype.PURCHASE ? "INVOICE" : "QUOTATION"), Line_Char) + DotmatrixPrint.Bold_Off);
                //line
                DotmatrixDataAlignment.PrintLine(rdr, Line_Char);
                //continue total
                rdr.WriteLine(DotmatrixPrint.Bold_On + DotmatrixDataAlignment.GetFormatedText((PurchaseEntry.PurchaseEntrytype == PurchaseEntrytype.PURCHASE ? "INVOICE" : "QUOTATION") + ": " + PurchaseEntry.RefNumber, columnSize[0], AlignmentTypes.Suffix) +
            ...
            private void TaxDetailTable(IList<TaxTable> lTaxTable, DataTable totalTable, string PrintPaper, PurchaseTaxType Type)
            {
                int numtax = Global.Company.SalesTaxAccountMaps.Count;
                numtax = Type == PurchaseTaxType.INTER ? (numtax > 2 ? 2 : numtax) : (numtax > 1 ? 1 : numtax);
                ...
                {
                    if ((Type == PurchaseTaxType.INTER && (Map.AccountId == 31 || Map.AccountId == 32)) || (Type == PurchaseTaxType.INTRA && Map.AccountId == 28))
                    {
                        rdr.Write(
