...
{
    internal class PurchasePrintSaveA5A4
    {
        public string fileName;
        // Dot Matrix Print // Laser Print //
        readonly String[] PurchaseDetailsTableColumnName = new String[]
        {
        "#", "Item Description","UOM", "HSN/SAC Code","Batch No", "Exp Date", "Rate","MRP", "Qty","Free", "Dis %", "Dis Amount", "Sub Total", "%", "Amount","Line Total"
        };
        public enum PurchaseDetailsTableColumn
        {
            SNO, DESC, UOM, HSN, BATCH, BATCHEXPDATE, RATE, MSRP, QTY, FREE, DISPER, DISAMOUNT, SUNTOTAL, TAXPER, TAXAMOUNT, LTOTAL
        }
        ...
        public void ExportToFileOrPrint(long PurchaseEntryId, string PrintPaper, string fileExtension, bool isPrint)
        {
            ...
            DataTable PurchaseDataTableTotal = new DataTable();
            DataTable PurchaseDataTable = new DataTable();
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.SNO], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.DESC], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.UOM], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.HSN], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.BATCH], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.BATCHEXPDATE], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.RATE], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.MSRP], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.QTY], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.FREE], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.DISPER], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.DISAMOUNT], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.SUNTOTAL], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.TAXPER], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.TAXAMOUNT], typeof(string));
            PurchaseDataTable.Columns.Add(PurchaseDetailsTableColumnName[(int)PurchaseDetailsTableColumn.LTOTAL], typeof(string));
            if (PurchaseEntry.PurchaseDetails.Count != 0)
            {
                ...
                try
                {
                    foreach (PurchaseDetails PurchaseDetails in PurchaseEntry.PurchaseDetails.OrderBy(x => x.Id))
                    {
                        ...
                        string PurchaseTax = "\n";
                        string subTotal = PurchaseDetails.PurchasePrice == 0 ? (PurchaseDetails.PurchasePrice * PurchaseDetails.Quantity).ToString("F") : (PurchaseDetails.PurchasePrice * PurchaseDetails.Quantity).ToString("F");
                        SubTotalForTax = PurchaseDetails.PurchasePrice == 0 ? (PurchaseDetails.PurchasePrice * PurchaseDetails.Quantity) : (PurchaseDetails.PurchasePrice * PurchaseDetails.Quantity);
                        var overAll = PurchaseDetails.PurchasePrice == 0 ? (PurchaseDetails.PurchasePrice * PurchaseDetails.Quantity) : (PurchaseDetails.PurchasePrice * PurchaseDetails.Quantity);
                        SubTotal = SubTotal + overAll;
                        //Tax map
                        foreach (TaxDetail Taxes in lPurchaseDetails.TaxDetails)
                        {
                            PurchaseTax += Global.Company.SalesTaxAccountMaps.FirstOrDefault(x => x.MapId == (Taxes.CatalogItemSalesTaxMap != null ? Taxes.CatalogItemSalesTaxMap.SalesTaxMapId : Taxes.ItemSalesTaxMap.SalesTaxMapId))!.Name + " @" + Taxes.TaxRate.ToString() + "% ";
                        }
                        //Discount
                        if (lPurchaseDetails.Discounts.Count > 0)
                        {
                            PurchaseTax += "\nDiscount @" + lPurchaseDetails.Discounts.Sum(X => X.Discount).ToString() + "%";
                            subTotal += "\n(" + Math.Round(lPurchaseDetails.Discounts.Sum(X => X.DiscountAmount), 2).ToString("F") + ")\n";
                            subTotal = PurchaseDetails.PurchasePrice == 0 ? ((PurchaseDetails.PurchasePrice * PurchaseDetails.Quantity) - lPurchaseDetails.Discounts.Sum(X => X.DiscountAmount)).ToString("F") : ((PurchaseDetails.PurchasePrice * PurchaseDetails.Quantity) - lPurchaseDetails.Discounts.Sum(X => X.DiscountAmount)).ToString("F");
                            SubTotalForTax = PurchaseDetails.PurchasePrice == 0 ? ((PurchaseDetails.PurchasePrice * PurchaseDetails.Quantity) - lPurchaseDetails.Discounts.Sum(X => X.DiscountAmount)) : ((PurchaseDetails.PurchasePrice * PurchaseDetails.Quantity) - lPurchaseDetails.Discounts.Sum(X => X.DiscountAmount));
                            Discount = Discount + lPurchaseDetails.Discounts.Sum(X => X.DiscountAmount);
                        }
                        count++;
                        //Tax details
                        foreach (LineLevelPurchaseTaxDetail SalesTax in lPurchaseDetails.TaxDetails)
                        {
                            TaxTable TaxTable = null;
                            //TaxDetail Taxes = lPurchaseDetails.TaxDetails.FirstOrDefault(x => x.CatalogItemSalesTaxMap.SalesTaxMapId == SalesTax.MapId);
                            //if (Taxes == null)
                            //{
                        ...
                        if (lPurchaseDetails.TaxDetails.Count > 0)
                        {
                            TaxTotal = TaxTotal + lPurchaseDetails.TaxDetails.Sum(X => X.Amount);
                        }
                        double TaxAmount = PdfDataAlignment.ProductTaxPercentagePurchase(lPurchaseDetails.TaxDetails, (PurchaseTaxType)PurchaseEntry.PurchaseEntrytype);
                        //create new row
                        DataRow PurchaseDataTableNewRow = PurchaseDataTable.NewRow();
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.SNO] = count.ToString();
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.UOM] = PurchaseDetails.Product.UOM.ToString();
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.DESC] = PurchaseDetails.Product.Name.ToString() + PurchaseTax;
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.HSN] = PurchaseDetails.Product.HSNCode == null ? "" : PurchaseDetails.Product.HSNCode.ToString();
                        if (PurchaseDetails.BatchNo != null)
                        {
                            PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.BATCH] = PurchaseDetails.BatchNo.ToString();
                            PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.BATCHEXPDATE] = PurchaseDetails.ExpDate.ToString(Global.Company.DateFormat);
                        }
                        else
                        {
                            PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.BATCH] = "";
                            PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.BATCHEXPDATE] = "";
                        }
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.RATE] = PurchaseDetails.PurchasePrice == 0 ? PurchaseDetails.PurchasePrice.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)) : PurchaseDetails.PurchasePrice.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision));
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.QTY] = PurchaseDetails.Quantity.ToString(TextUtils.DecimalPlace(Global.Company.QuantityPricision));
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.FREE] = PurchaseDetails.FreeQuantity.ToString();
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.DISPER] = lPurchaseDetails.Discounts.Count > 0 ? lPurchaseDetails.Discounts.Sum(x => x.Discount).ToString("F") : "0.00";
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.DISAMOUNT] = lPurchaseDetails.Discounts.Count > 0 ? lPurchaseDetails.Discounts.Sum(x => x.DiscountAmount).ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)) : TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision);
                        double Stotal = double.Parse(subTotal.ToString());
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.SUNTOTAL] = Stotal.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision));
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.TAXPER] = PdfDataAlignment.ProductTaxPercentagePurchase(PurchaseDetails.TaxDetails, (PurchaseTaxType)PurchaseEntry.PurchaseEntrytype).ToString("F");
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.TAXAMOUNT] = lPurchaseDetails.TaxDetails.Count > 0 ? lPurchaseDetails.TaxDetails.Sum(x => x.Amount).ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)) : TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision);
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.LTOTAL] = Math.Round(PurchaseDetails.Amount , 2).ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision));
                        PurchaseDataTableNewRow[(int)PurchaseDetailsTableColumn.MSRP] = PurchaseDetails.Msrp.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision));
                        //Add row
                        PurchaseDataTable.Rows.Add(PurchaseDataTableNewRow);
                    }
                    PurchaseDataTableTotal.Columns.Add("1", typeof(string));
                    PurchaseDataTableTotal.Columns.Add("2", typeof(string));
                    PurchaseDataTableTotal.Columns.Add("3", typeof(string));
                    PurchaseDataTableTotal.Columns.Add("4", typeof(string));
                    PurchaseDataTableTotal.Columns.Add("5", typeof(string));
                    //Grand total
                    PurchaseDataTableTotal.Rows.Add(new object[] { "Grant Total", (SubTotal - Discount).ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)), "      ", TaxTotal.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)), LintTotal.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)) });
                    double LintTotalTrans = Math.Round(LintTotal, 2);
                    for (int i = 0; i < PurchaseEntry.PurchaseAdditionalTransactions.Count; i++)
                    {
                        if (PurchaseEntry.PurchaseAdditionalTransactions[i].Action == AdditionalTransactionAction.CR)
                        {
                            LintTotalTrans = LintTotalTrans - PurchaseEntry.PurchaseAdditionalTransactions[i].Amount;
                        }
                        else
                        {
                            LintTotalTrans = LintTotalTrans + PurchaseEntry.PurchaseAdditionalTransactions[i].Amount;
                        }
                        var TransType = PurchaseEntry.PurchaseAdditionalTransactions[i].Type == AdditionalTransactionType.PERCENT ? " @" + PurchaseEntry.PurchaseAdditionalTransactions[i].Value + "%" : "";
                        //Additional trans
                        PurchaseDataTableTotal.Rows.Add(new object[] { PurchaseEntry.PurchaseAdditionalTransactions[i].Name + TransType, Math.Round(PurchaseEntry.PurchaseAdditionalTransactions[i].Amount, 2).ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)), "       ", "", Math.Round(LintTotalTrans, 2).ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)) });
                    }
                    //Round off
                    double RoundedTotal = LintTotalTrans;
                    double RoundoffAmount = Rounds > 0 ? RoundOff(RoundedTotal) : 0;
                    PurchaseDataTableTotal.Rows.Add(new object[] { "Round off", RoundoffAmount.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)), "      ", "", (RoundedTotal + RoundoffAmount).ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)) });
                }
            ...
            if (PurchaseDataTable.Rows.Count != 0)
            {
                ...
                {
                    PurchasePrintSaveA5A4New PurchasePrintSaveA5A4New = new PurchasePrintSaveA5A4New();
                    //Customer need
                    PurchasePrintSaveA5A4New.GeneratePDF(lTaxTable, PurchaseDataTable, PurchaseDataTableTotal, PurchaseEntry, PrintPaper, "pdf", isPrint);
                    //Standrad
                    //GeneratePDF(lTaxTable,PurchaseDataTable, PurchaseDataTableTotal, PurchaseEntry, PrintPaper, "pdf", isPrint);
                }
                else if (fileExtension == "Dotmatrix")
                {
                    fileName = PurchaseEntry.PurchaseEntrytype == PurchaseEntrytype.PURCHASE ? "PurchaseInvoice" : PurchaseEntry.PurchaseEntrytype == PurchaseEntrytype.RETURN ? "PurchaseReturnInvoice" : "";
                    PurchasePrintSaveDotmatrixA5A4 PurchasePrintSaveDotmatrixA4Portraid = new PurchasePrintSaveDotmatrixA5A4(this);
                    PurchasePrintSaveDotmatrixA4Portraid.GenerateDotmarixTxt(lTaxTable, PurchaseDataTable, PurchaseDataTableTotal, PurchaseEntry, PrintPaper, isPrint);
                }
            }
        }
        ...
        public void GeneratePDF(IList<TaxTable> lTaxTable, DataTable dataTable, DataTable totalTable, PurchaseEntry PurchaseEntry, string PrintPaper, string fileExtension, bool isPrint)
        {
            using (MemoryStream myMemoryStream = new MemoryStream())
            {
                ...
                PdfPTable PurchaseHeader = PdfDataAlignment.PurchaseHeader(PurchaseEntry);
                PdfPTable PurchaseMiniHeader = PdfDataAlignment.PurchaseMiniHeader(PurchaseEntry);
                ...
                double HeaderTableHeight = PurchaseHeader.TotalHeight + PurchaseMainTable.TotalHeight + DummyTable.TotalHeight;
                ...
                string RefNumber = (PurchaseEntry.PurchaseEntrytype == PurchaseEntrytype.PURCHASE ? "Invoice: " : ": ") + PurchaseEntry.RefNumber;
                ...
                for (int i = 0; i < rows; i++)
                {
                    ...
                    if (TotalWorkingOnPageH >= A4Height)
                    {
                        ...
                        table.SetWidths(widths);
                        pdfDoc.Add(PurchaseMiniHeader);
                        pdfDoc.Add(MiniDummyTableWithoutBoard);

                        HeaderTableHeight = PurchaseMiniHeader.TotalHeight + MiniDummyTableWithoutBoard.TotalHeight;
                ...
                if (TotalWorkingOnPageH > A4Height)
                {
                    ...
                    TotalWorkingOnPageH = MiniDummyTableWithoutBoard.TotalHeight + PurchaseMiniHeader.TotalHeight + PdfDataAlignment.CalculatePdfTableHeight(totalPdfTable);
                }
                ...
                pdfDoc.Close();
                PdfGeneration.SaveMemoryStream(myMemoryStream, (PurchaseEntry.PurchaseEntrytype == PurchaseEntrytype.PURCHASE ? "SaleInvoice" : "SaleQuote"), fileExtension, isPrint, PrintPaper == "A5 LANDSCAPE" ? PaperTypes.A5_LANDSCAPE : PrintPaper == "A4 LANDSCAPE" ? PaperTypes.A4_LANDSCAPE : PaperTypes.A4_PORTRAIT);
            }
        }
            ...
            if (SaleEntry.SaleDetails.Count != 0)
            {
                ...
                try
                {
                    foreach (SaleDetail SaleDetails in SaleEntry.SaleDetails)
                    {
                        ...
                        SaleDetailsTableNewRow[(int)SalePrePrintDetailsTableColumn.RATE] = SaleDetails.OverridePrice == 0 ? SaleDetails.Price.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)) : SaleDetails.OverridePrice.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision));
                        ...
                        SaleDetailsTableNewRow[(int)SalePrePrintDetailsTableColumn.DISAMOUNT] = lSaleDetail.Discounts.Count > 0 ? lSaleDetail.Discounts.Sum(x => x.DiscountAmount).ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)) : TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision);
                        ...
                        SaleDetailsTableNewRow[(int)SalePrePrintDetailsTableColumn.TAXAMOUNT] = lSaleDetail.TaxDetails.Count > 0 ? lSaleDetail.TaxDetails.Sum(x => x.Amount).ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)) : TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision);
                        SaleDetailsTableNewRow[(int)SalePrePrintDetailsTableColumn.LTOTAL] = Math.Round(SaleDetails.Amount, 2).ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision));
                    ...
                    //Grand total
                    SaleDataTableTotal.Rows.Add(new object[] { "Grant Total", (SubTotal - Discount).ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)), TaxWiseAmountTotals[0], TaxWiseAmountTotals[1], LintTotal.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)) });
                    ...
                    //Round off
                    SaleDataTableTotal.Rows.Add(new object[] { "Round off", SaleEntry.RoundOff.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)), "      ", "", SaleEntry.NetAmount.ToString(TextUtils.DecimalPlace(Global.Company.PrimaryCurrency.RoundingPrecision)) });
                }
                catch (DocumentException dex)
                {
