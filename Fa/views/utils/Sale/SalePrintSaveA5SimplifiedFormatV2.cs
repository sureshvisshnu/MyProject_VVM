...
{
    public class SalePrintSaveA5SimplifiedFormatV2
    {
        ...
        private const float ITEM_ROW_HEIGHT = 15f;
        private const float TABLE_HEADER_HEIGHT = 18f;
        private const float FOOTER_REQUIRED_HEIGHT = 70f;
        ...
        public void ExportToFileOrPrint(long SalesId, string PrintPaper, string fileExtension, bool isPrint, Entrytype entrytype, bool isLandscape = false, bool isForWE = false)
        {
            ...
            if (SaleEntry.SaleDetails.Count != 0)
            {
                ...
                try
                {
                    foreach (SaleDetail SaleDetails in SaleEntry.SaleDetails.OrderBy(x => x.PrintOrderNo))
                    {
                        count++;
        ...
        private PdfPTable CreateItemsTable(bool isLandscape)
        {
        ...
        public void GeneratePDF(DataTable dataTable, SaleEntry saleEntry, string PrintPaper, string fileExtension, bool isPrint, bool isLandscape, double TotalAmount, Entrytype entrytype, bool isWE = false)
        {
            using (MemoryStream myMemoryStream = new MemoryStream())
            {
                ...
                PageNumberHelper pageNumberHelper = new PageNumberHelper
                {
                ...
                string title = entrytype == Entrytype.QUOTE ? "ESTIMATE" : "SALES INVOICE";
                ...
                pdfDoc.Add(CustomerDetails(saleEntry));
                pdfDoc.Add(new Paragraph(" "));

                PdfPTable pageTable = CreateItemsTable(isLandscape);

                pageTable = CreateSalesMainTableHeader(pageTable, dataTable);

                for (int i = 0; i < dataTable.Rows.Count; i++)
                {
                    bool isTotal = (i == dataTable.Rows.Count - 1);
                ...
                pdfDoc.Add(pageTable);
                pdfDoc.Add(new Paragraph(" "));

                pdfDoc.Add(
                    AmtInWordsColumn(TotalAmount));
        ...
        private void AddItemRow(PdfPTable table, DataRow row, bool isTotal)
        {
            for (int j = 0; j < row.Table.Columns.Count; j++)
            {
                ...
                cell.MinimumHeight = ITEM_ROW_HEIGHT;
        ...
        private float GetRemainingHeight(Document document, PdfWriter writer)
        {
            return writer.GetVerticalPosition(true) - document.BottomMargin;
        }

        private bool HasSpaceForNextRow(Document document, PdfWriter writer)
        {
            return GetRemainingHeight(document, writer) >
                   (ITEM_ROW_HEIGHT + FOOTER_REQUIRED_HEIGHT);
        }
    }
}
