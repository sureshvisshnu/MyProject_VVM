...
namespace fa.views.controls.ComboTreeView
{
    /// <summary>
    ...
    public class ComboTreeBox : DropDownControlBase
    {
        ...
        public ComboTreeBox()
        {
            ...
            _dropDown.KeyPress += new KeyPressEventHandler(KeyPress_event);
            _dropDown.PreviewKeyDown += new PreviewKeyDownEventHandler(PreviewKeyDown_event);
        ...
        void TextChanged_event(object sender, EventArgs e)
        {
            OnTextChanged(e);
        }
        ...
        void KeyPress_event(object sender, KeyPressEventArgs e)
        {
            OnKeyPress(e);
        }
        ...
        {
            MouseEventArgs MouseEventArgs = new MouseEventArgs(System.Windows.Forms.MouseButtons.Left, 1, 187, 8, 0);
        ...
        protected override void OnPaintContent(DropDownPaintEventArgs e)
        {
           ...
           // TextRenderer.DrawText(e.Graphics, text, Font, txtBounds, ForeColor, BackColor);
        ...
        public bool CheckedChange = true;
        ...
        void nodes_AfterCheck(object sender, ComboTreeNodeEventArgs e)
        {
            ...
            if (CheckedChange)
            {
                if (_cascadeCheckState)
                {
                    ...
                    if (_recurseDepth == 1)
                    {
                        IEnumerator<ComboTreeNode> enumerator = ComboTreeNodeCollection.GetNodesRecursive(e.Node.Name == "All" && e.Node.Text == "All"?this.Nodes:e.Node.Nodes, false);
                ...
                //
                if (e.Node.Name != "All" && !Recursive)
                {
                    bool Check = AllNodeCheckingStatus(e, this.Nodes, true);
                    ComboTreeNode tns = this.Nodes.FindByName("All", StringComparison.CurrentCultureIgnoreCase, true);
                    if (tns!=null && tns.Checked != Check)
                    {
                        CheckedChange = false;
                        tns.Checked = Check;
                        CheckedChange = true;
        ...
        private bool AllNodeCheckingStatus(ComboTreeNodeEventArgs Args, ComboTreeNodeCollection ComboBoxTreeNode, bool AllNodeCheckingStatusInnodeCheck)
        {
