...
{
    public class PurchaseDoubleEntryManager
    {
        static string SupplierEntryForPurchaseText = "For Purchase/bill {0}";
        static string PurchasesAccountEntryForInvoiceText = "For Purchase/bill {0}, from {1}";
        static string AdditionalTransactionText = "{0} applied against Purchase/bill {1}, from {2}";
        ...
        private static volatile PurchaseDoubleEntryManager instance;
        private static object syncRoot = new Object();
        PurchaseDoubleEntryManager()
        {
        }
        public static PurchaseDoubleEntryManager Instance
        {
            get
            {
                if (instance == null)
                {
                    ...
                    {
                        if (instance == null)
                            instance = new PurchaseDoubleEntryManager();
                    }
                }
                return instance;
            }
        }
        public void RecordPurchase(PurchaseEntry Purchase, AccountMasterContext Context)
        {
                DeletePurchase(Purchase, Context);
                AddPurchase(Purchase, Context);
        }
        /*
         * Deleteing all exisiting entries for the given company 
         */
        public void DeletePurchase(PurchaseEntry Purchase, AccountMasterContext Context)
        {
            Company CurrentCompany = Context.Companies.Where(p => p.CompanyId == Purchase.CompanyId).First<Company>();
            if (CurrentCompany == null)
            {
                Exception e = new Exception(CurrentCompanyNullMsg);
                throw e;
            }
            List<DayBook> xx = Context.DoubleEntries.Where(x => x.TransactionType == DaybookTransactionType.Purchase && x.CompanyId == CurrentCompany.CompanyId && x.ReferenceTrasnactionId == "" + Purchase.RefNumber).ToList();
            Context.DoubleEntries.RemoveRange(xx);
            Context.SaveChanges();
        }
        public void AddPurchase(PurchaseEntry Purchase, AccountMasterContext Context)
        {
            Company CurrentCompany = Context.Companies.Where(p => p.CompanyId == Purchase.CompanyId).First<Company>();
            if (CurrentCompany == null)
            {
        ...
        public DayBook getCreditSupplierEntry(PurchaseEntry Purchase)
        {
            ...
            dBook.Date = Purchase.RefDate;
            dBook.Credit(Purchase.TotalAmount);
            dBook.Description = string.Format(SupplierEntryForPurchaseText, Purchase.RefNumber);
            dBook.TransactionType = DaybookTransactionType.Purchase;
        ...
        public List<DayBook> getPurchaseAdditionalTransactionEntries(PurchaseEntry Purchase)
        {
            ...
            foreach (PurchaseAdditionalTransaction PurchaseAdditionalTransaction in Purchase.PurchaseAdditionalTransactions)
            {
                ...
                dBook.CostCenterId = Purchase.CostCenterId;
                dBook.Date = Purchase.RefDate;
                dBook.Debit(PurchaseAdditionalTransaction.Amount * (PurchaseAdditionalTransaction.Action == AdditionalTransactionAction.CR ? 1 : -1));
